...
title Install JA MES Tool

echo ========================================================
echo         INSTALL JA MES TOOL FOR WINDOWS
echo ========================================================
echo.
...
set "SOURCE_DIR="
if exist "%SCRIPT_DIR%\ja_mes_tool.exe" (
    set "SOURCE_DIR=%SCRIPT_DIR%"
) else if exist "%SCRIPT_DIR%\build\windows\x64\runner\Release\ja_mes_tool.exe" (
    set "SOURCE_DIR=%SCRIPT_DIR%\build\windows\x64\runner\Release"
) else if exist "%SCRIPT_DIR%\dist\ja_mes_tool.exe" (
    set "SOURCE_DIR=%SCRIPT_DIR%\dist"
)

if not defined SOURCE_DIR (
    echo [ERROR] Cannot find ja_mes_tool.exe.
    echo Place install.bat next to ja_mes_tool.exe
...
set "TARGET_DIR=%LOCALAPPDATA%\Programs\JA_MES_Tool"
...
:: Never force-stop another portable instance or interrupt unsaved data
powershell -NoProfile -Command "$exe = Join-Path $env:TARGET_DIR 'ja_mes_tool.exe'; if (Get-Process ja_mes_tool -ErrorAction SilentlyContinue | Where-Object { $_.Path -eq $exe }) { Write-Host 'Close the installed app before continuing.'; exit 1 }"
...
robocopy "%SOURCE_DIR%" "%TARGET_DIR%" /E /R:1 /W:1 /XD logs /XF update_config.json config.json config.ini *.log *.key *.zip SHA256SUMS.txt >nul
if errorlevel 8 (
...
:: Desktop may be redirected to OneDrive; resolve its Windows known folder
set "START_MENU_DIR=%APPDATA%\Microsoft\Windows\Start Menu\Programs\JA MES Tool"
if not exist "%START_MENU_DIR%" mkdir "%START_MENU_DIR%"
set "START_MENU_APP_LNK=%START_MENU_DIR%\JA MES Tool.lnk"
set "START_MENU_UNINST_LNK=%START_MENU_DIR%\Uninstall JA MES Tool.lnk"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$u.Description = 'Uninstall JA MES Tool'; " ^
  "$u.Save();"
if errorlevel 1 goto install_error
...
echo.
echo ========================================================
echo     [DONE] JA MES TOOL INSTALLED SUCCESSFULLY!
echo ========================================================
echo - Installation directory: %TARGET_DIR%
echo - Desktop shortcut: JA MES Tool.lnk
echo - Menu Start: Programs \ JA MES Tool
...
set /p RUN_APP="Launch JA MES Tool now? (Y/N, default Y): "
if "%RUN_APP%"=="" set "RUN_APP=Y"
if /i "%RUN_APP%"=="yes" set "RUN_APP=Y"
if /i "%RUN_APP%"=="y" (
    start "" "%TARGET_DIR%\ja_mes_tool.exe"
)
exit /b 0
